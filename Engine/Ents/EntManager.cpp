...
void EntUpdatePointers(std::vector<std::unique_ptr<Ent>> &Ent,Brush *Br,Material *Mt,GpuLights *L,DISPLAY *D,Vec3 *CamPos,MapKey *Key,MapTransition *Tr,EngineMouse *M){
    for (int i = 0; i < Ent.size(); i++) {
        if (Ent[i]) {
            Ent[i]->SetPointers(Br,Mt,L,D,CamPos,Key,&Ent,Tr,M,Ent[i].get());
            }
    }
}
